...
const CS2 = () => {
  const cs2Game = gamesConfig.games.find(game => game.id === 'cs2');
  const cs2Plans = cs2Game?.plans || [];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-red-900 to-purple-900">
      ...
      {isSectionEnabled('gamePages', 'hero') && (
        <section className="relative bg-gradient-to-br from-red-900 via-slate-800 to-red-900 text-white py-20 overflow-hidden">
          ...
          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="text-6xl mb-6 animate-bounce">{cs2Game?.icon || '🔫'}</div>
            <Badge className="mb-4 bg-red-600/20 text-red-300 border-red-500/50">
              Professional CS2 Hosting
            </Badge>
            ...
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              {cs2Game?.description || 'Experience competitive CS2 with our optimized servers, anti-cheat protection, and tournament-ready configurations.'}
            </p>
          </div>
        </section>
      )}
      ...
      {/* CS2 Plans */}
      {cs2Plans.length > 0 && isSectionEnabled('gamePages', 'plans') && (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              ...
              <p className="text-xl text-gray-300 max-w-2xl mx-auto">
                Choose your perfect CS2 server plan for competitive gaming
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {cs2Plans.map((plan, index) => (
                <Card 
                  key={plan.id} 
                ...
                >
                  {plan.popular && (
                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                      <Badge className="bg-gradient-to-r from-red-600 to-orange-600 text-white px-4 py-1">Most Popular</Badge>
                    </div>
                  )}
                  
                  <CardHeader className="text-center">
                    <CardTitle className="text-white group-hover:text-red-400 transition-colors">{plan.name}</CardTitle>
